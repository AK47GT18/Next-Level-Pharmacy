...
class ConfirmationModal
{
    ...
    public function render(): string
    {
        ...
        return <<<HTML
        <div id="{$this->id}" class="fixed inset-0 z-[100] hidden items-center justify-center bg-black/40 p-4 backdrop-blur-sm modal-backdrop" style="display: none;">
        ...
        (function(){
            ...
            // Expose show and hide functions globally
            window['{$this->id}_show'] = show;
            window['{$this->id}_hide'] = hide;
            ...
            // Dispatch custom event for confirm - external handlers can prevent default hide
            modal.querySelector('[data-modal-confirm]')?.addEventListener('click', (e) => {
                modal.dataset.confirmed = 'true';
                // Only hide if no external onclick handler was set (check if default behavior)
                const event = new CustomEvent('modal:confirm', { bubbles: true, cancelable: true });
                const cancelled = !modal.dispatchEvent(event);
                if (!cancelled && !e.target.dataset.preventAutoHide) {
                    hide();
                }
            });
        })();
