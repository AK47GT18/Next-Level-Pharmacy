...
class PathHelper
{
    ...
    public static function loadCSS(string $path): string
    {
        if (self::fileExists($path)) {
            $version = filemtime(self::getBasePath() . '/' . ltrim($path, '/'));
            return '<link rel="stylesheet" href="' . self::asset($path) . '?v=' . $version . '">';
        }
    ...
    public static function loadJS(string $path, bool $defer = false): string
    {
        if (self::fileExists($path)) {
            ...
            return '<script src="' . self::asset($path) . '"' . $deferAttr . '?v=' . $version . '></script>';
        }
