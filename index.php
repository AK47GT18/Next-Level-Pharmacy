...
// Removed hardcoded admin role to allow RBAC to function correctly.
...
// --- Centralized RBAC Routing Check ---
$isAdmin = ($_SESSION['role'] ?? '') === 'admin';
$restrictedPages = ['reports', 'settings'];

if (!$isAdmin && in_array($currentPage, $restrictedPages)) {
	// Exception: Allow access to user-profile view in settings even for non-admins
	$view = $_GET['view'] ?? null;
	if (!($currentPage === 'settings' && $view === 'user-profile')) {
		setFlash('error', 'You do not have permission to access this page.');
		header('Location: index.php?page=dashboard');
		exit;
	}
}
// --- End Centralized RBAC ---
?>
<!doctype html>
<html lang="en">
...
<head>
	...
	<style>
		...
		* {
		...
		/* Radical Mobile Performance Mode: Disable blur and use static opacity */
		@media (max-width: 768px) {
			...
			/* Disable complex shadows on mobile to save GPU repaints */
			.shadow-lg,
			.shadow-xl,
			.shadow-2xl {
	...
	<div class="flex h-[100dvh] overflow-hidden">
		...
		<div class="flex-1 flex flex-col overflow-hidden">
			...
			<main id="main-content" class="flex-1 overflow-auto p-4 md:p-8 custom-scrollbar"
				style="-webkit-overflow-scrolling: touch; overscroll-behavior-y: bounce;">
				<?php
