...
try {
    ...
    $recentSales = array_map(function ($sale) {
        ...
        // Actual fixed time display
        $timeStr = $created->format('g:i A');

        if ($diff->d > 0) {
            $timeAgo = $diff->d . ' d' . ($diff->d > 1 ? 's' : '') . ' ago';
        } elseif ($diff->h > 0) {
            $timeAgo = $diff->h . ' h' . ($diff->h > 1 ? 's' : '') . ' ago';
        } else {
            $timeAgo = max(1, $diff->i) . ' m' . ($diff->i > 1 ? 's' : '') . ' ago';
        }

        return [
            'invoice' => '#' . str_pad($sale['id'], 3, '0', STR_PAD_LEFT), // More compact
            'customer' => 'Sale by ' . htmlspecialchars($sale['sold_by'] ?? 'Unknown'),
            'amount' => (float) $sale['total_amount'],
            'status' => 'Completed',
            'date' => $timeStr . ' (' . $timeAgo . ')',
