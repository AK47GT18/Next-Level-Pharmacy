...
<div class="space-y-6 animate-slide-in">
    ...
    <div class="glassmorphism rounded-2xl shadow-lg p-6 border border-gray-100">
        <div class="flex items-center justify-between mb-6">
            ...
            <button data-open-add-user
                class="px-5 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl hover:shadow-lg transition-all font-semibold flex items-center gap-2">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b-2 border-gray-100">
                        <th class="text-left py-4 px-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Name
                        </th>
                        <th class="text-left py-4 px-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Email
                        </th>
                        <th class="text-left py-4 px-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Phone
                        </th>
                        <th class="text-left py-4 px-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Role
                        </th>
                        <th class="text-left py-4 px-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Status
                        </th>
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php foreach ($users as $u): ?>
                        <tr class="hover:bg-blue-50/50 transition group" data-user-id="<?= $u['id'] ?>">
                            ...
                            <td class="py-4 px-4">
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-bold bg-blue-100 text-blue-700">
                            ...
                            <td class="py-4 px-4">
                                ...
                                $isActive = (time() - $lastActivity) < 3600; // Active if activity within last hour
                                ?>
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold <?= $isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500' ?>">
                                    <i
                                        class="fas fa-circle text-[6px] <?= $isActive ? 'text-emerald-500' : 'text-gray-400' ?>"></i>
                            ...
                            <td class="py-4 px-4">
                                <div class="flex gap-2">
                                    <button
                                        class="edit-user-btn p-2 w-8 h-8 flex items-center justify-center rounded-lg text-blue-600 hover:bg-blue-100 hover:text-blue-800 transition"
                                        data-user-id="<?= $u['id'] ?>" title="Edit User">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button
                                        class="delete-user-btn p-2 w-8 h-8 flex items-center justify-center rounded-lg text-rose-600 hover:bg-rose-100 hover:text-rose-800 transition"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('.delete-user-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                // Open the modal using the global show function
                if (window['deleteUserModal_show']) {
                    window['deleteUserModal_show']();
                } else {
        ...
        // ✅ Single, persistent confirm listener to avoid memory leaks/accumulated actions
        const deleteModal = document.getElementById('deleteUserModal');
        if (deleteModal) {
            deleteModal.addEventListener('modal:confirm', async () => {
                ...
                // Hide modal using the global hide function we exposed
                window['deleteUserModal_hide']();
            });
        }
    });
</script>